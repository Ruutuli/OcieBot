  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      if (attempt === MAX_RETRIES - 1) {
        // Last attempt failed
        logger.error(`MongoDB connection error (failed after ${MAX_RETRIES} attempts): ${errorMessage}`);
        process.exit(1);
      } else {
        // Retry with delay
        const delay = RETRY_DELAYS[attempt];
        logger.error(`MongoDB connection error (attempt ${attempt + 1}/${MAX_RETRIES}): ${errorMessage}`);
